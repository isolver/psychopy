...
class PSTBox(Device):
    ...
    _serial_slots = ['port', 'baud', 'bytesize', 'parity', 'stopbits', '_serial', '_timeout', '_rx_buffer',
                  '_parser_config', '_parser_state', '_event_count',
                  '_byte_diff_mode','_custom_parser','_custom_parser_kwargs',
                  '_nlamps', '_lamp_state', '_streaming_state', '_button_query_state',
                  '_state', '_button_bytes', '_button_events']
    __slots__ = [e for e in _serial_slots]
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._nlamps = 5
        self._lamp_state = N.repeat(0, self._nlamps).astype('bool')
        ...
        self._button_bytes = list((2**N.arange(0, 5)).astype(N.uint8))
        self._button_events = ['press', 'release']
    ...
    def _update_state(self):
        ...
        if self._state[6] or \
                (not N.array_equal(self._lamp_state, state_old[:5])):
            update_lamp_state = True
        else:
            update_lamp_state = False
        ...
        state_new_bin = (2**N.arange(9))[state_new]
        self.write(chr(N.sum(state_new_bin)))

        state_new[6] = False
        self._state = state_new

    def getState(self):
        # FIXME Why can't we return the numpy array? Spawns error msg.
    ...
    def _createByteChangeSerialEvent(self, logged_time, read_time, prev_byte, new_byte):
        ...
        if new_byte != 0:  # Button was pressed
            ...
            button_event = self._button_events[0]
        else:  # Button was released
            button = self._button_bytes.index(prev_byte)
            button_event = self._button_events[1]
...
class PSTBoxByteChangeEvent(DeviceEvent):
    _newDataTypes = [
            ('port', N.str, 32),
            ('prev_byte', N.uint8),
            ('current_byte', N.uint8),
